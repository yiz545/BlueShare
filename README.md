...
- 終端機快捷鍵控制（無需切換視窗）
- 退出時自動銷毀 Aggregate Device（不會在系統累積殭屍裝置）
...
- **🎙️ 藍牙麥克風物理限制**：同時啟用多支耳機的麥克風時，macOS 會強制切換到 HFP (Hands-Free Profile) 通話模式，音質會降為單聲道低位元率。**這是藍牙硬體層級限制，無法用軟體破解**。也因此 macOS 原生版的「對講迴路」（把麥克風送回兩支耳機）在實務上會嚴重劣化音樂品質，目前版本不啟用此路由 — 錄音改為直接錄系統麥克風（內建為主），音樂播放維持 A2DP 高音質。
